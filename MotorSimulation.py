...
class MotorSimulation:
    ...
    def update(self, setpoint, dt):
        ...
        target_angular_acceleration = max(min(target_angular_acceleration, self.max_angular_acceleration), -self.max_angular_acceleration)

        # Calculate the change in acceleration
        acceleration_change = target_angular_acceleration - self.current_angular_acceleration

        # Limit the change in acceleration
        limited_acceleration_change = max(min(acceleration_change, self.max_angular_acceleration_change * dt), -self.max_angular_acceleration_change * dt)
        ...
        self.angular_velocity = max(min(self.angular_velocity, self.motor.max_angular_velocity), -self.motor.max_angular_velocity)
