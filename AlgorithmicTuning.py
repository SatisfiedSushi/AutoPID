...
class PID_Controller:
    ...
    def set_new_target(self, target):
        self.target = target
        self.accumulator = 0  # Reset the accumulator for the new target

    def adjust_signal(self, feedback_value):
        # Normalize the target and feedback value to be within -180 to 180 degrees
        target_normalized = (self.target + 180) % 360 - 180
        feedback_normalized = (feedback_value + 180) % 360 - 180

        # Calculate the shortest path error considering the circular nature
        error = target_normalized - feedback_normalized
        if error > 180:
            error -= 360  # Adjust error if it's the shorter path across the 180/-180 boundary
        elif error < -180:
            error += 360  # Adjust error if it's the shorter path across the 180/-180 boundary

        # PID calculations
        self.accumulator += error
        derivative = (error - (target_normalized - self.last_reading)) / self.sample_rate

        # Update the signal with PID output
        self.signal = self.kp * error + self.ki * self.accumulator + self.kd * derivative

        # Limit the signal to the maximum allowed value
        if self.signal > self.max_signal:
            self.signal = self.max_signal
        elif self.signal < -self.max_signal:
            self.signal = -self.max_signal

        # Update the last reading for the next derivative calculation
        self.last_reading = feedback_normalized
    ...
    def auto_tune(self, system_simulator, min_input, max_input):
        ...
        for i in range(max_iterations):
            ...
            for target in check_targets:
                ...
                time_list, signal_list, temp_x, target_list = self.run_simulation(system_simulator, 500,
                                                                                  [[0, target], [250, 0]])
            ...
            print('Iteration {}, Kp = {}, Ki = {}, Kd = {}, Error = {}'.format(i, self.kp, self.ki, self.kd,
                                                                               current_error))
            ...
            if plot_current_iteration or (i == 1):
                ...
                fig.suptitle(
                    'Iteration = ' + str(i) + ', Kp = ' + str(self.kp) + ', Ki = ' + str(self.ki) + ', Kd = ' + str(
                        self.kd))
            ...
            if step_size < min_step_size:
                ...
                if constant_to_optimize == 0:
                    ...
                    self.kp = round(self.kp, 2)
                elif constant_to_optimize == 1:
                    self.ki += 10 * step_size * direction
                    self.ki = round(self.ki, 2)
                elif constant_to_optimize == 2:
            ...
            if constant_to_optimize == 3 and current_error < 1:
                print('Final Values: Kp = {}, Ki = {}, Kd = {}, Error = {}'.format(i, self.kp, self.ki, self.kd,
                                                                                   current_error))
...
fig.suptitle(
    'Simulated System: Mass = {}, k = {},  c = {}'.format(round(system_simulator.mass, 2), round(system_simulator.k, 2),
                                                          round(system_simulator.c, 2)))
